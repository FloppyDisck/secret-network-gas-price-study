...
fn test_permit() -> Result<()> {
    ...
    // Test that permits work
    {
        ...
        let query: QueryAnswer = query(&contract, msg, Some(100))?;
    ...
    // Test for bad permits
    {
        let mut bad_permit = permit.clone();
        bad_permit.params.key = "bad_key".to_string();
    ...
    // Test that permit key banning works
    {
        ...
        handle(&msg, &contract, "a", Some(GAS), Some("test"), None, &mut vec![], Some(100))?;
    }

    // Test that permits key is banned
    {
        let msg = QueryMsg::Permit {
            permit
        };
